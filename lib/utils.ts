...
export function getServerSortIndex(serverInfo: any): number {
  // 只使用哪吒后台 servers 表里的 display_index 排序。
  // 不再读取 id、name 或括号内备用编号，避免把数据库 id 当成排序值。
  const rawDisplayIndex = Number(
    serverInfo?.display_index ?? serverInfo?.DisplayIndex ?? serverInfo?.displayIndex ?? 0,
  )

  return Number.isFinite(rawDisplayIndex) ? rawDisplayIndex : 0
}
