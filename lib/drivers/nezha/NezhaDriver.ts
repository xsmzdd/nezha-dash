...
const isTruthyEnv = (value: string | undefined) => value?.trim().toLowerCase() === "true"
...
const isHiddenForGuest = (server: any) =>
  [
    ["hide_for_guest"],
    ["hideForGuest"],
    ["HideForGuest"],
    ...
    ["Host", "hideForGuest"],
    ["Host", "HideForGuest"],
  ].some((path) => toBooleanFlag(getNestedValue(server, path)))
...
export class NezhaDriver extends BaseDriver {
  ...
  private async getGuestVisibleServerIds(): Promise<Set<number> | null> {
    try {
      ...
      if (!Array.isArray(resData?.result)) return null

      return new Set(
        resData.result
          .map((server: any) => Number(server?.id))
          .filter((serverId: number) => Number.isFinite(serverId)),
      )
    } catch (error) {
      console.warn("Unable to fetch Nezha guest visible server list:", error)
      return null
    }
  }

  private async canShowServerToGuest(serverId: number, server?: any): Promise<boolean> {
    if (isTruthyEnv(getEnv("ForceShowAllServers"))) return true
    if (server && isHiddenForGuest(server)) return false

    const guestVisibleServerIds = await this.getGuestVisibleServerIds()
    return guestVisibleServerIds === null || guestVisibleServerIds.has(serverId)
  }
  ...
  async getServers(): Promise<ServerApi> {
    ...
    const forceShowAllServers = isTruthyEnv(getEnv("ForceShowAllServers"))
    const guestVisibleServerIds = forceShowAllServers ? null : await this.getGuestVisibleServerIds()
    const nezhaDataFiltered = forceShowAllServers
      ? nezhaData
      : nezhaData.filter(
          (element) =>
            !isHiddenForGuest(element) &&
            (guestVisibleServerIds === null || guestVisibleServerIds.has(element.id)),
        )
    ...
    data.result = nezhaDataFiltered.map(
      (element: MakeOptional<NezhaAPI, "ipv4" | "ipv6" | "valid_ip">) => {
        ...
        const rawDisplayIndex =
          (element as any).display_index ??
          (element as any).DisplayIndex ??
          (element as any).displayIndex ??
          0
        element.display_index = Number(rawDisplayIndex) || 0
  ...
  async getServerDetail(serverId: number): Promise<NezhaAPI> {
    ...
    const forceShowAllServers = isTruthyEnv(getEnv("ForceShowAllServers"))
    const guestVisibleServerIds = forceShowAllServers ? null : await this.getGuestVisibleServerIds()
    const detailData = detailDataList
      .filter(
        (element) =>
          forceShowAllServers ||
          (!isHiddenForGuest(element) &&
    ...
    if (!detailData) {
      throw new DriverOperationError(this.name, "getServerDetail", "Server is hidden for guest")
    }
  ...
  protected async onGetServerMonitor(serverId: number): Promise<NezhaAPIMonitor[]> {
    ...
    if (!(await this.canShowServerToGuest(serverId))) {
      throw new DriverOperationError(this.name, "getServerMonitor", "Server is hidden for guest")
    }
  ...
  protected async onGetServerIP(serverId: number): Promise<string> {
    ...
    if (!(await this.canShowServerToGuest(serverId, server))) {
      throw new DriverOperationError(this.name, "getServerIP", "Server is hidden for guest")
    }
