...
import { MapIcon, Squares2X2Icon, ViewColumnsIcon } from "@heroicons/react/20/solid"
...
import ServerCardPro from "@/components/ServerCardPro"
import Switch from "@/components/Switch"
import ShinyText from "@/components/ui/shiny-text"
import getEnv from "@/lib/env-entry"
import { cn, getServerSortIndex } from "@/lib/utils"
...
const sortServersByDisplayIndex = (servers: any[]) => {
  return [...servers].sort((a, b) => {
    const displayIndexDiff = getServerSortIndex(b) - getServerSortIndex(a)
    return displayIndexDiff !== 0 ? displayIndexDiff : Number(a.id) - Number(b.id)
  })
}
...
const ServerList = memo(function ServerList({
  ...
  if (inline === "2") {
    return (
      <section ref={containerRef} className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {servers.map((serverInfo) => (
          <ServerCardPro key={serverInfo.id} serverInfo={serverInfo} />
        ))}
      </section>
    )
  }
...
export default function ServerListClient() {
  ...
  const [inline, setInline] = useState<string>("2")

  useEffect(() => {
    const inlineState = localStorage.getItem("inline")
    if (inlineState === "0" || inlineState === "1" || inlineState === "2") {
  ...
  const { filteredServers, uniqueTags, tagCountMap } = useMemo(() => {
    if (!result.length) {
      return {
        filteredServers: [],
        uniqueTags: [defaultTag],
        tagCountMap: {},
      }
    }
    ...
    let nextFilteredServers = filterServersByTag(filteredServersByStatus, tag, defaultTag)

    if (filter) {
      nextFilteredServers = sortServersByNetwork(nextFilteredServers)
    }

    return {
      filteredServers: nextFilteredServers,
      uniqueTags: tags,
      tagCountMap: getTagCounts(filteredServersByStatus),
    }
  }, [result, status, tag, filter])
    ...
    <>
      <section className="flex w-full items-center gap-2 overflow-hidden">
        ...
        <button
          type="button"
          onClick={() => {
            const newInline = inline === "0" ? "1" : inline === "1" ? "2" : "0"
            setInline(newInline)
        ...
        >
          {inline === "2" ? (
            <Squares2X2Icon className="size-[13px]" />
          ) : (
