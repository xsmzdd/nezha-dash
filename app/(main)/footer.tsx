...
const baseTextStyles =
  "text-[17px] font-light tracking-tight text-neutral-600/70 dark:text-neutral-300/70"
...
export default function Footer() {
  ...
  return (
    <footer className="mx-auto flex w-full max-w-[1500px] items-center justify-between py-2">
      <section className="flex flex-col gap-1.5">
        {/* 渐变标题 */}
        <h2 className="bg-gradient-to-r from-yellow-400 via-orange-400 to-red-500 bg-clip-text font-semibold text-[19px] text-transparent tracking-wide">
          加钱道人全球监控面板
        </h2>

        {/* 版权 + 图标组 */}
        <div className={`flex items-center gap-2 ${baseTextStyles}`}>
      ...
      {/* 快捷键提示 */}
      <kbd className="pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border bg-muted px-2 font-mono text-[13px] text-muted-foreground">
        {isMac ? <span className="text-base">⌘</span> : "Ctrl "}K
      </kbd>
    </footer>
  )
}
